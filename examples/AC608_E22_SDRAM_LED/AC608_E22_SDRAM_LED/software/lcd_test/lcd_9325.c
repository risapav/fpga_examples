...
uint16_t LCD_RD_DATA(void) {
	uint16_t t;
	IOWR_ALTERA_AVALON_PIO_DIRECTION(LCD_DB_BASE, 0x0000);
#if LCD_FAST_IO==1 //快速IO
	LCD_RS_SET;
	LCD_CS_CLR;
	//读取数据(读寄存器时,并不需要读2次)
...
//	usleep(20);
	...
	LCD_CS=1;
#endif
	IOWR_ALTERA_AVALON_PIO_DIRECTION(LCD_DB_BASE, 0xffff);
	IOWR_ALTERA_AVALON_PIO_DATA(LCD_DB_BASE, 0xffff);
	//全部输出高
	return t;
}
...
void LCD_WriteRAM_Prepare(void) {
	if (DeviceCode == 0X9341)
		LCD_WR_REG(0x2C); //ILI9341是以0X2C开始的
	else
		LCD_WR_REG(R34);
}
...
uint16_t LCD_ReadPoint(uint16_t x, uint16_t y) {
	...
	IOWR_ALTERA_AVALON_PIO_DATA(LCD_DB_BASE, 0xffff);
	//全部输出高
	if (DeviceCode == 0X9325 || DeviceCode == 0X4535 || DeviceCode == 0X4531
			|| DeviceCode == 0X8989 || DeviceCode == 0XB505)
...
void LCD_Init(void) {
	...
	IOWR_ALTERA_AVALON_PIO_DATA(LCD_BL_BASE, 1);
	//点亮背光
	LCD_Clear(WHITE);
}
